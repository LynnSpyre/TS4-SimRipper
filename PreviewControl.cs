...
namespace TS4SimRipper
{
    public partial class Form1 : Form
    {
        ...
        public static void LoadBONDMorph(GEOM baseMesh, BOND boneDelta, RIG rig)
        {
            LoadBONDMorph(new GEOM[] { baseMesh }, boneDelta, rig, true);
        }
        public static void LoadBONDMorph(GEOM baseMesh, BOND boneDelta, RIG rig, bool updateRig)
        {
            LoadBONDMorph(new GEOM[] { baseMesh }, boneDelta, rig, updateRig);
        }
        public static void LoadBONDMorph(GEOM[] baseMeshes, BOND boneDelta, RIG rig)
        {
            LoadBONDMorph(baseMeshes, boneDelta, rig, true);
        }
        public static void LoadBONDMorph(GEOM[] baseMeshes, BOND boneDelta, RIG rig, bool updateRig)
        {
        ...
        public static GEOM LoadBGEOMorph(GEOM baseMesh, BGEO morph, int lod)
        {
            ...
            for (int i = 0; i < morphMesh.numberVertices; i++)
            {
                ...
                {
                    float vertWeight = 1f;
                    if (morphMesh.copyFaceMorphs) vertWeight = ((morphMesh.getTagval(i) & 0x003F0000) >> 16) / 63f;
        ...
        private void GetCurrentModel(bool skinOnly, int desiredLevelOfDetail)
        {
            ...
            if (!skinOnly)
            {
                ...
                currentRig = GetTS4Rig(currentSpecies, adjustedAge);
                GEOM head = null;
                //GEOM collar = null;
                ...
                for (int i = 0; i < outfit.Length; i++)
                {
                    ...
                    fullInfo += outfit[i].PartName + " Colorshift:  0x" + colorShifts[i].ToString("X16") + " (" + packname + ")" + Environment.NewLine + Environment.NewLine;
                }
                ...
                for (int i = 0; i < outfit.Length; i++)
                {
                    ...
                    if (tgis.Length > 0)
                    {
                        ...
                        foreach (TGI tgi in tgis)
                        {
                            ...
                            if (outfit[i].BodyType == BodyType.Head && geom.ShaderHash != (uint)SimShader.Phong)
                            {
                                if ((currentFrame & outfit[i].gender) == 0)
                                {
                                    geom = LoadDMapMorph(geom, frameModifier[0], frameModifier[1]);
                                }
                                head = geom;
                            }
                ...
                {
                    currentSkin = DisplayablePelt(currentPelt, currentSculptOverlay);
                }
                if (currentSkin == null)
                {
        ...
        public Image CompositeTexture()
        {
            if (currentSkin == null && currentMakeup == null && currentClothing == null) return null;
            Image currentTexture = new Bitmap(currentSkin.Width, currentSkin.Height);
            using (Graphics g = Graphics.FromImage(currentTexture))
            {
                g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBicubic;
                if (currentSkin != null)
        ...
        private void SaveModelMorph(MeshFormat format)
        {
            ...
            {
                DAE dae = new DAE(geomList.ToArray(), nameList.ToArray(), currentRig, false, CleanDAE_checkBox.Checked);
